...
export const processReminders = functions.pubsub.schedule('every 5 minutes').onRun(async (context) => {
    ...
    try {
        ...
        for (const userDoc of usersSnap.docs) {
            ...
            let updatesNeeded: any = {};

            // ---------------------------------------------------------
            // A) Lunch Reminder
            // ---------------------------------------------------------
            if (
                entry.clockInManual &&            // Clocked in
                ...
                if (enableEmails && email) {
                    ...
                    await notificationService.sendEmail(email, 'Reminder: Lunch break not logged', body);
                }

                if (enableSMS && smsOptIn && phone) {
                    await notificationService.sendSMS(phone, `Reminder: lunch break not logged today. Please update your time in the app.`);
                }
            ...
            if (
                ...
                if (enableEmails && email) {
                    ...
                    await notificationService.sendEmail(email, 'Reminder: You are still clocked in', body);
                }

                if (enableSMS && smsOptIn && phone) {
                    await notificationService.sendSMS(phone, `Reminder: you have not clocked out today. Please update your time in the app.`);
                }
            ...
            if (
                ...
                const clockInMoment = moment.tz(`${currentDateString} ${entry.clockInManual}`, "YYYY-MM-DD HH:mm", timezone);
                ...
                if (hoursElapsed >= longShiftThreshold) {
                    ...
                    if (enableEmails && email) {
                        ...
                        await notificationService.sendEmail(email, 'Reminder: Long shift detected', body);
                    }

                    if (enableSMS && smsOptIn && phone) {
                        await notificationService.sendSMS(phone, `You have been clocked in over ${longShiftThreshold} hours. Please confirm or clock out.`);
                    }
